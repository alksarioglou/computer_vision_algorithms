...
def main():
    ...
    if args.part == 'all' or args.part == 'ncc':
        ...
        plot_correlation(left_crop, right_crop, corr, 40)
    ...
    if args.part == 'all' or args.part == 'stereo':
        ...
        #         1. In triangulate(): If the projected x-coordinates of a point in the left and right images are identical,
        #            the calculated disparity will be zero and a triangulation is not possible.
        #         2. In compute_ncc_fast(): If all pixels of a patch have the same intensity, the standard deviation of that patch will
        #            be zero, which invalidates the normalization of that patch. By definition, we set the NCC in those cases to be 0.
...
def triangulate(x_left, x_right, y, m_width, m_height, camera_parameters):
    ...
    x_left = x_left - m_width / 2.0
    x_right = x_right - m_width / 2.0
    y = y - m_height / 2.0

    kx = m_width / aperture_x
    ky = m_height / aperture_y

    disparity = x_left - x_right

    with np.errstate(divide='ignore', invalid='ignore'):  # suppress zero division warning
        points[:, 0] = baseline * x_left / disparity
        points[:, 1] = baseline * kx / ky * y / disparity
        points[:, 2] = baseline * kx * focal_length / disparity
...
def compute_ncc(gray_left, gray_right, mask_halfwidth):
    ...
    for y in range(mask_halfwidth, m_height - mask_halfwidth):
        ...
        for x_l in range(mask_halfwidth, m_width - mask_halfwidth):
            patch_left = gray_left[y - mask_halfwidth: y + mask_halfwidth + 1,
                         x_l - mask_halfwidth: x_l + mask_halfwidth + 1]

            # Normalize the left patch
            mean_left = patch_left.mean()

            patch_left_min_mean = patch_left - mean_left
            var_left = np.square(patch_left_min_mean).mean()
            dev_left = np.sqrt(var_left)
            patch_left_norm = patch_left_min_mean / dev_left

            # Loop over patches in the right image
            for x_r in range(mask_halfwidth, m_width - mask_halfwidth):
                patch_right = gray_right[y - mask_halfwidth: y + mask_halfwidth + 1,
                              x_r - mask_halfwidth: x_r + mask_halfwidth + 1]

                # Normalize the right patch
                mean_right = patch_right.mean()

                patch_right_min_mean = patch_right - mean_right
                var_right = np.square(patch_right_min_mean).mean()
                dev_right = np.sqrt(var_right)

                patch_right_norm = patch_right_min_mean / dev_right

                # Compute correlation
                corr[y - mask_halfwidth, x_l - mask_halfwidth, x_r - mask_halfwidth] = (patch_left_norm *
                                                                                        patch_right_norm).mean()
...
def compute_ncc_fast(gray_left, gray_right, mask_halfwidth):
    ...
    i = 0
    for y in range(-mask_halfwidth, mask_halfwidth + 1):
        for x in range(-mask_halfwidth, mask_halfwidth + 1):
            # Shift the image to get different elements of the patch
            patches_left[:, :, i] = np.roll(np.roll(gray_left, -y, axis=0), -x, axis=1)
            patches_right[:, :, i] = np.roll(np.roll(gray_right, -y, axis=0), -x, axis=1)
            i += 1

    # Compute mean for each patch
    patches_left_mean = patches_left.mean(-1, keepdims=True)
    patches_right_mean = patches_right.mean(-1, keepdims=True)

    # Subtract mean
    patches_left_min_mean = patches_left - patches_left_mean
    patches_right_min_mean = patches_right - patches_right_mean

    # Compute standard deviation for each patch
    patches_left_var = np.square(patches_left_min_mean).mean(-1, keepdims=True)
    patches_left_dev = np.sqrt(patches_left_var)

    patches_right_var = np.square(patches_right_min_mean).mean(-1, keepdims=True)
    patches_right_dev = np.sqrt(patches_right_var)

    # Normalize the patches
    with np.errstate(divide='ignore', invalid='ignore'):  # to suppress zero divison warnings
        patches_left_norm = patches_left_min_mean / patches_left_dev
        patches_right_norm = patches_right_min_mean / patches_right_dev

    # Compute correlation.
    # Hint: This can be computed as a matrix multiplication. Check np.matmul and np.transpose
    corr = np.matmul(patches_left_norm, np.transpose(patches_right_norm, (0, 2, 1))) / (2 * mask_halfwidth + 1) ** 2

    # Filter using the variance of the left patch, if needed
    corr = corr * (patches_left_dev > 0)
    return corr[mask_halfwidth:m_height - mask_halfwidth,
                mask_halfwidth:m_width - mask_halfwidth,
                mask_halfwidth:m_width - mask_halfwidth]
...
def define_points_3d(gray_left, gray_right, mask_halfwidth, camera_parameters):
    ...
    corr = compute_ncc_fast(gray_left, gray_right, mask_halfwidth)

    # restrict search space
    left_ids = np.arange(mask_halfwidth, m_width - mask_halfwidth).reshape((1, new_width, 1))
    right_ids = np.arange(mask_halfwidth, m_width - mask_halfwidth).reshape((1, 1, new_width))

    keep = right_ids < left_ids

    corr_valid = corr * keep

    # find the corresponding points x_right for x_left
    x_left = np.arange(mask_halfwidth, m_width - mask_halfwidth).reshape(1, new_width).repeat(new_height, 0)

    x_right = np.argmax(corr_valid, axis=2) + mask_halfwidth
    x_right_val = corr_valid.max(axis=2)
    x_right[x_right_val <= 0.] = x_left[x_right_val <= 0.]

    y = np.arange(mask_halfwidth, m_height - mask_halfwidth).reshape(new_height, 1).repeat(new_width, 1)

    # Triangulate
    points3d = triangulate(x_left.reshape(-1), x_right.reshape(-1), y.reshape(-1), m_width, m_height, camera_parameters)

    points3d = points3d.reshape((new_height, new_width, 3))
    return points3d
